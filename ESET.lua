...
function tdcli_update_callback(data)
  ...
  if data.ID == "UpdateNewMessage" then
	...
	if not database:get('openchat') then
database:setex('openchat', 120, true)
...
The *Number* of *Warnings* user : `[]] .. warns .. "/" .. sencwarn .. "]`", 1, "md")
      ...
      if is_owner(msg.sender_user_id_, msg.chat_id_) and idf:match("-100(%d+)") then
        local text = text:gsub("حذف مقام", "Demote")
      ...
      if is_momod(msg.sender_user_id_, msg.chat_id_) and idf:match("-100(%d+)") then
        local text = text:gsub("حذف مسدود", "Unban")
      ...
      if is_momod(msg.sender_user_id_, msg.chat_id_) and idf:match("-100(%d+)") then
        local text = text:gsub("سکوت", "Muteuser")
      ...
      if is_momod(msg.sender_user_id_, msg.chat_id_) and idf:match("-100(%d+)") then
        local text = msg.content_.text_:gsub("لغو سکوت", "Unmuteuser")
...
------------------------------
---open chat for all groups---
------------------------------
if text:match("^/openchats$") and is_momod(msg) then
  local chat = database:smembers('bot:groups')
  for i=1, #chat do
  openChat(chat[i])
  end
send(100481835, 0, 1, "chats reloaded", 1, 'md')
  end
...
local ap = {string.match(text, "^[/!#]openchat (.*)$")}
-----------
openChat(ap[2])
-----------
send(msg.chat_id_, 0, 1, "chat "..ap[2].." has been reloaded", 1, 'md')
end
        -------------------------------------------------
        ...
        local text = msg.content_.text_:gsub("حذف مقام", "Demote")
        ...
        local text = msg.content_.text_:gsub("حذف مسدود", "Unban")
        ...
        local text = msg.content_.text_:gsub("سکوت", "Muteuser")
        ...
        if is_admin(msg.sender_user_id_) and (text:match("^[Gg]plist$") or text:match("^لیست گروه ها$")) then
...
[]] .. expire .. [[
        ...
        if is_momod(msg.sender_user_id_, msg.chat_id_) and idf:match("-100(%d+)") and (text:match("^[Mm]odlist$") or text:match("^لیست مدیران$")) and check_user_channel(msg) then
          local hash = "bot:momod:" .. msg.chat_id_
          local list = database:smembers(hash)
          if database:get("lang:gp:" .. msg.chat_id_) then
        ...
        if is_momod(msg.sender_user_id_, msg.chat_id_) and idf:match("-100(%d+)") and (text:match("^[Mm]utelist$") or text:match("^لیست سکوت$")) and check_user_channel(msg) then
          local hash = "bot:muted:" .. msg.chat_id_
          local list = database:smembers(hash)
          if database:get("lang:gp:" .. msg.chat_id_) then
        ...
        if is_momod(msg.sender_user_id_, msg.chat_id_) and idf:match("-100(%d+)") and (text:match("^[Oo]wner$") or text:match("^[Oo]wnerlist$") or text:match("^لیست صاحبان$")) and check_user_channel(msg) then
          local hash = "bot:owners:" .. msg.chat_id_
          local list = database:smembers(hash)
          if not database:get("lang:gp:" .. msg.chat_id_) then
        ...
        if is_momod(msg.sender_user_id_, msg.chat_id_) and idf:match("-100(%d+)") and (text:match("^[Bb]anlist$") or text:match("^لیست مسدود$")) and check_user_channel(msg) then
          local hash = "bot:banned:" .. msg.chat_id_
          local list = database:smembers(hash)
          if database:get("lang:gp:" .. msg.chat_id_) then
...
> <b>ID</b> : <code>]] .. result.id_ .. "</code>"
        ...
        if is_admin(msg.sender_user_id_) and idf:match("-100(%d+)") and (text:match("^[Aa]ddhelper$") or text:match("^دعوت هلپر$")) then
          function Invhelper(extra, result)
            sendBotStartMessage(result.id_, msg.chat_id_, "start", dl_cb)
          end
          resolve_username("HelperKiLaGeRBot", Invhelper)
        end
       ...
       if is_momod(msg.sender_user_id_, msg.chat_id_) and idf:match("-100(%d+)") and (text:match("^[Dd]el all rmsg$") or text:match("^پاکسازی همه پیام ها$")) and check_user_channel(msg) then
          local d = database:ttl("waitforfeed:" .. msg.chat_id_)
          local time = math.floor(d / 60) + 1
          if database:get("waitforfeed:" .. msg.chat_id_) then
            if database:get("lang:gp:" .. msg.chat_id_) then
              send(msg.chat_id_, msg.id_, 1, "■ Dear Manager, this is possible every 12 hours. Please apply again after " .. time .. " minutes ■", 1, "md")
            else
              send(msg.chat_id_, msg.id_, 1, "■ مدیر گرامی این امکان هر 12 ساعت یکبار ممکن میباشد\nلطفا بعد از " .. time .. " دقیقه مجدد اقدام کنید ■", 1, "md")
            end
          else
            ...
            database:setex("bot:feed" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 80, true)
            database:setex("waitforfeed:" .. msg.chat_id_, 43200, true)
          end
        end
        if is_momod(msg.sender_user_id_, msg.chat_id_) and idf:match("-100(%d+)") and (text:match("^[Ll]ink$") or text:match("^لینک$")) and check_user_channel(msg) then
          local link = database:get("bot:group:link" .. msg.chat_id_)
          if link then
            if database:get("lang:gp:" .. msg.chat_id_) then
        ...
        if is_momod(msg.sender_user_id_, msg.chat_id_) and idf:match("-100(%d+)") and (text:match("^[Ss]upport$") or text:match("^گروه پشتیبانی$")) and check_user_channel(msg) then
...
`
        ...
        if is_momod(msg.sender_user_id_, msg.chat_id_) and text:match("^قفل گروه (%d+) (%d+) (%d+)$") and idf:match("-100(%d+)") and check_user_channel(msg) then
          local matches = {
            string.match(text, "^قفل گروه (%d+) (%d+) (%d+)$")
          }
          local hour = string.gsub(matches[1], "h", "")
          local num1 = tonumber(hour) * 3600
          local minutes = string.gsub(matches[2], "m", "")
        ...
        if is_momod(msg.sender_user_id_, msg.chat_id_) and idf:match("-100(%d+)") and text:match("^قفل گروه (%d+) (%d+)$") and check_user_channel(msg) then
          local matches = {
            string.match(text, "^قفل گروه (%d+) (%d+)$")
          }
          local hour = string.gsub(matches[1], "h", "")
          local num1 = tonumber(hour) * 3600
          local minutes = string.gsub(matches[2] or 0, "m", "")
        ...
        if is_momod(msg.sender_user_id_, msg.chat_id_) and idf:match("-100(%d+)") and text:match("^قفل گروه (%d+)$") and check_user_channel(msg) then
          local matches = {
            string.match(text, "^(قفل گروه) (%d+)$")
          }
          local hour = string.gsub(matches[2], "h", "")
          local num1 = tonumber(hour) * 3600
          if 1 <= tonumber(matches[2]) then
...
Sentences have over `]] .. sensspam[2] .. "` Character will Delete !", 1, "md")
        ...
        if is_momod(msg.sender_user_id_, msg.chat_id_) and (text:match("^[Cc]lean (.*)$") or text:match("^پاکسازی (.*)$")) and check_user_channel(msg) then
          ...
          if txt[2] == "banlist" or txts[2] == "لیست مسدود" and idf:match("-100(%d+)") then
          ...
          if is_sudo(msg) and (txt[2] == "members" or txts[2] == "اعضضا" and idf:match("-100(%d+)")) then
            do
          ...
          if is_owner(msg.sender_user_id_, msg.chat_id_) and (txt[2] == "modlist" or txts[2] == "لیست مدیران" and idf:match("-100(%d+)")) then
            if database:get("lang:gp:" .. msg.chat_id_) then
          ...
          if txt[2] == "ownerlist" or txts[2] == "لیست صاحبان" and idf:match("-100(%d+)") then
            if database:get("lang:gp:" .. msg.chat_id_) then
          ...
          if txt[2] == "viplist" or txts[2] == "لیست عضوهای ویژه" and idf:match("-100(%d+)") then
            if database:get("lang:gp:" .. msg.chat_id_) then
          ...
          if txt[2] == "mutelist" or txts[2] == "لیست سکوت" and idf:match("-100(%d+)") then
        ...
        if is_momod(msg.sender_user_id_, msg.chat_id_) and idf:match("-100(%d+)") and (text:match("^[Ss]ettings") or text:match("^تنظیمات")) and check_user_channel(msg) then
          ...
          elseif (text:match("^[Ss]ettingspv$") or text:match("^تنظیمات پیوی$")) and is_admin(msg.sender_user_id_) and idf:match("-100(%d+)") then
            if database:get("lang:gp:" .. msg.chat_id_) then
        ...
        if is_momod(msg.sender_user_id_, msg.chat_id_) and idf:match("-100(%d+)") and (text:match("^$") or text:match("^$")) and check_user_channel(msg) then
          if database:get("getidstatus" .. msg.chat_id_) == "Photo" then
            showiden = "Photo"
            showidfa = "عکس"
...
All your *Warnings* Has Been Cleared !]], 1, "md")
        ...
        if is_momod(msg.sender_user_id_, msg.chat_id_) and idf:match("-100(%d+)") and (text:match("^[Rr]ules$") or text:match("^قوانین$")) then
        ...
        if is_admin(msg.sender_user_id_) and idf:match("-100(%d+)") and text:match("^شارژ (%d+)$") then
          do
            local a = {
              string.match(text, "^(شارژ) (%d+)$")
            }
            if a[2]:match("^0$") then
              if not database:get("lang:gp:" .. msg.chat_id_) then
        ...
        if is_admin(msg.sender_user_id_) and idf:match("-100(%d+)") and text:match("^شارژ نامحدود$") then
        ...
        if is_admin(msg.sender_user_id_) and idf:match("-100(%d+)") and text:match("^شارژ ازمایشی$") then
...
Please *specify* the group and the group reputation ! .]], 1, "md")
              else
                send(msg.chat_id_, msg.id_, 1, "• گروه " .. (chat and chat.title_ or "") .. " به لیست مدیریتی ربات اضافه شد !\nلطفا صاحب گروه و میزان اعتبار گروه را تعیین نمایید !", 1, "md")
              end
              local fname = result.first_name_ or ""
              local lname = result.last_name_ or ""
        ...
        if is_momod(msg.sender_user_id_, msg.chat_id_) and idf:match("-100(%d+)") and text:match("^[Dd]el (%d+)$") then
          ...
          if msg.chat_id_:match("^-100") then
            if tonumber(matches[2]) > 1000 or 1 > tonumber(matches[2]) then
              if database:get("lang:gp:" .. msg.chat_id_) then
                pm = "• Please use a Number <b>Greater</b> than <code>1</code> and less than <code>1000</code> !"
              else
                pm = "• لطفا از عددي بزرگتر از 1 و کوچکتر از 1000 استفاده کنيد !"
              end
        ...
        if is_momod(msg.sender_user_id_, msg.chat_id_) and idf:match("-100(%d+)") and msg.reply_to_message_id_ ~= 0 and (text:match("^[Pp]in$") or text:match("^سنجاق$")) and check_user_channel(msg) then
          local id = msg.id_
          local msgs = {
            [0] = id
          }
        ...
        if is_owner(msg.sender_user_id_, msg.chat_id_) and idf:match("-100(%d+)") then
         ...
         if text:match("^[Hh]elp$") and is_momod(msg.sender_user_id_, msg.chat_id_) then
         ...
         if text:match("^راهنما$") and is_momod(msg.sender_user_id_, msg.chat_id_) then
                local help = io.open("./Help/helpfa.txt", "r")
                local helptime = 30
                local a = ( help:read("*a") )
                database:setex('helptime:'..msg.chat_id_, helptime, true)
                send(msg.chat_id_, msg.id_, 1, a, 1, 'md')
        ...
        if is_leader(msg) and (text:match("^[Uu]pdate$") or text:match("^بروزرسانی$")) then
          if not database:get("lang:gp:" .. msg.chat_id_) then
            send(msg.chat_id_, msg.id_, 1, "انجام شده", 1,"md")
          else
        ...
        if is_Naji(msg) and text:match("^[Ww]hat[Ss]ource$") then
          if database:get("lang:gp:" .. msg.chat_id_) then
            send(msg.chat_id_, msg.id_, 1, "bot", 1, "md")
          else
            send(msg.chat_id_, msg.id_, 1, "bot", 1, "md")
          end
        end
        if is_Naji(msg) and text:match("^[Ii]nfo$") then
          ...
          if is_leader(msg) and text:match("^[Ff]orceupdate$") then
            if not database:get("lang:gp:" .. msg.chat_id_) then
              send(msg.chat_id_, msg.id_, 1, "انجام شد", 1, "md")
            else
              send(msg.chat_id_, msg.id_, 1, "done", 1, "md")
            end
          end
